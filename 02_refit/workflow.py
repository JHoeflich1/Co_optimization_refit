...
class Workflow:
    ...
    def replace_protocol(self, old_protocol, new_protocol, update_paths_only=False):
        ...
        if new_id in [x.id for x in self._protocols]:
            raise ValueError(
                "A protocol with the same id already exists in this workflow: ", new_id
            )
        ...
        for labelled_molecule in labelled_molecules:
            for parameter_key in parameter_gradient_keys:
                ...
                if not labelled_parameters:
                    ...
                    # Extract raw parameters under the tag
                    tagged_data = labelled_molecule[parameter_key.tag]

                    if hasattr(tagged_data, "items"):
                        print(f"  Matches found: {len(tagged_data.items())}")
                        for smirks, match in tagged_data.items():
                            print(f"    - SMIRKS: {smirks}")
                            print(f"      Param: {match.parameter_type}")
                    elif isinstance(tagged_data, list):
                        print(f"  Matches found (list): {len(tagged_data)}")
                        for i, match in enumerate(tagged_data):
                            try:
                                print(f"    - Param[{i}]: {match.parameter_type}")
                            except AttributeError:
                                print(f"    - Param[{i}]: {match}")
                    else:
                        print(f"  Unknown data type under tag: {type(tagged_data)}")
